...
export default function Sectoin1(props){
    return(
        <div className = {`${styles.container}`}>
            ...
            <div data-aos-offset = {-300*scroll} data-aos = 'fade-up' data-aos-duration = '3000' style = {{position: 'relative'}}>
                <div className = {`${styles.bell}`} style = {{position: 'absolute', top: '-20px', left: '160px'}}>
                    <Image priority  src = '/../public/about_sec1_bell.png' width = '100px' height = '100px'/>
                </div>
                <p className = {`${styles.heading}`}>We Shape Visionary Ideas So That It Connects Dots in People life</p>
                <br/>
            </div>
            <p data-aos-offset = {-300*scroll} data-aos = 'fade-up' data-aos-duration = '3000' className = {`${styles.text}`}>Building Amazing products using cutting-edge technologies mixed with physchology</p>
            <br/>
